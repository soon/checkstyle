 ...
 */
public abstract class AbstractChecker extends AutomaticBean
    ...
    /** General message to use when an exception occurs while processing a file. */
    private static final String EXCEPTION_WHILE_PROCESSING_FILE_MESSAGE =
            "Exception was thrown while processing ";

    /** General message to use when an error occurs while processing a file. */
    private static final String ERROR_WHILE_PROCESSING_FILE_MESSAGE =
            "Error was thrown while processing ";

    /** Logger for AbstractChecker. */
    private static final Log LOG = LogFactory.getLog(AbstractChecker.class);
    ...
    /**
     * Creates a new {@code AbstractChecker} instance.
    ...
    /**
     * Processes all given files with all FileSetChecks.
     * @param filteredFiles a list of files to process.
     * @throws CheckstyleException if error condition within Checkstyle occurs.
     * @noinspection ProhibitedExceptionThrown
     */
    protected abstract void processFilteredFiles(List<File> filteredFiles)
     ...
     * @noinspection ProhibitedExceptionThrown
     */
    protected abstract SortedSet<LocalizedMessage> processFile(File file)
            throws CheckstyleException;

    /**
     * Retrieves all fileSet checks for the given checker.
     * @return A list of fileSet checks.
     */
    public List<FileSetCheck> getFileSetChecks() {
        return Collections.unmodifiableList(fileSetChecks);
    }
    ...
    public int process(List<File> files) throws CheckstyleException {
        ...
        // Finish up
        // It may also log!!!
        finishFileSetChecksProcessing();

        // It may also log!!!
        destroyFileSetChecks();
    ...
    /**
     * Finishes file processing for each fileset check.
     */
    ...
    /**
     * Filters all files, required to process and caches them.
     * @param files A list of all files.
     * @return A list of files, that should be processed.
     * @throws CheckstyleException When something went wrong while checking a file.
     * @noinspection ProhibitedExceptionThrown
     */
    private List<File> filterAndCacheFilesToProcess(List<File> files) throws CheckstyleException {
        ...
        for (File file : files) {
            ...
            // processing. See https://github.com/checkstyle/checkstyle/issues/2285
                ...
                throw new CheckstyleException(EXCEPTION_WHILE_PROCESSING_FILE_MESSAGE
                        + file.getPath(), ex);
            }
            catch (Error error) {
                // We need to catch all errors to put a reason failure (file name) in error
                throw new Error(ERROR_WHILE_PROCESSING_FILE_MESSAGE + file.getPath(), error);
            }
        }
    ...
    protected void processFiles(List<File> files) throws CheckstyleException {
        final List<File> filesToProcess = filterAndCacheFilesToProcess(files);
        processFilteredFiles(filesToProcess);
    }

    /**
     * Processes the given file with all FileSetChecks.
     * @param file a file to process.
     * @throws CheckstyleException if error condition within Checkstyle occurs.
     * @noinspection ProhibitedExceptionThrown
     */
    protected void fireEventsAndProcessFile(File file) throws CheckstyleException {
        try {
            final String fileName = file.getAbsolutePath();
     ...
     * Processes a file with all FileSetChecks.
     * @param file a file to process.
     * @param filesetChecks a list of fileset checks to process.
     * @return a sorted set of messages to be logged.
     * @throws CheckstyleException if error condition within Checkstyle occurs.
     * @noinspection ProhibitedExceptionThrown
     */
    protected final SortedSet<LocalizedMessage> doProcessFile(
        File file, List<FileSetCheck> filesetChecks) throws CheckstyleException {
