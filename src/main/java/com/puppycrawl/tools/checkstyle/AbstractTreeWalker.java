...
public abstract class AbstractTreeWalker extends AbstractFileSetCheck
    ...
    protected SetMultimap<String, AbstractCheck> tokenToOrdinaryChecks =
        HashMultimap.create();

    /** Maps from token name to comment checks. */
    protected SetMultimap<String, AbstractCheck> tokenToCommentChecks =
            HashMultimap.create();

    /** Registered ordinary checks, that don't use comment nodes. */
    protected Set<AbstractCheck> ordinaryChecks = new HashSet<>();

    /** Registered comment checks. */
    protected Set<AbstractCheck> commentChecks = new HashSet<>();

    /** The ast filters. */
    private final Set<TreeWalkerFilter> filters = new HashSet<>();

    /** The sorted set of messages. */
    protected final SortedSet<LocalizedMessage> messages = new TreeSet<>();
    ...
    protected void processFiltered(File file, FileText fileText) throws CheckstyleException {
        ...
        System.err.println("processFiltered: " + (after - before) + "millis");
    }
     ...
     */
    protected abstract void walk(DetailAST ast, FileContents contents, AstState astState)
            throws CheckstyleException;
    ...
    public void finishCloning(FileSetCheck originalFileSetCheck) {
        ...
        AbstractTreeWalker originalTreeWalker = (AbstractTreeWalker) originalFileSetCheck;

        commentChecks = new HashSet<>();
        ordinaryChecks = new HashSet<>();

        Map<AbstractCheck, AbstractCheck> oldToCloneMap = new HashMap<>();
        for (AbstractCheck check : originalTreeWalker.commentChecks) {
            AbstractCheck copiedCheck = oldToCloneMap.computeIfAbsent(
                    check, CheckCloneService::cloneCheck);
            commentChecks.add(copiedCheck);
        }

        for (AbstractCheck check : originalTreeWalker.ordinaryChecks) {
            AbstractCheck copiedCheck = oldToCloneMap.computeIfAbsent(
                    check, CheckCloneService::cloneCheck);
            ordinaryChecks.add(copiedCheck);
        }

        tokenToCommentChecks = HashMultimap.create();
        originalTreeWalker.tokenToCommentChecks.forEach((name, check) -> {
            AbstractCheck copiedCheck = oldToCloneMap.computeIfAbsent(
                    check, CheckCloneService::cloneCheck);
            tokenToCommentChecks.put(name, copiedCheck);
        });

        tokenToOrdinaryChecks = HashMultimap.create();
        originalTreeWalker.tokenToOrdinaryChecks.forEach((name, check) -> {
            AbstractCheck copiedCheck = oldToCloneMap.computeIfAbsent(
                    check, CheckCloneService::cloneCheck);
            tokenToOrdinaryChecks.put(name, copiedCheck);
        });
    }
